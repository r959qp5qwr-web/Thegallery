...
REVOKE ALL ON ALL TABLES IN SCHEMA public FROM gallery_anon, gallery_auth;
...
CREATE POLICY operator_write ON operator_actions FOR INSERT TO gallery_auth
  WITH CHECK (app.is_operator() AND actor_account_id = app.account_id());
...
CREATE OR REPLACE FUNCTION app.publish_work(p_work_id uuid, p_intent_key text)
...
BEGIN
  ...
  IF w.lifecycle = 'published' THEN
    INSERT INTO write_intents (key, account_id, kind, subject_id, result)
      VALUES (p_intent_key, app.account_id(), 'publish_work', p_work_id, '{"outcome":"already_published"}');
  ...
  END IF;
  INSERT INTO write_intents (key, account_id, kind, subject_id, result)
    VALUES (p_intent_key, app.account_id(), 'publish_work', p_work_id, '{"outcome":"published"}');
  RETURN QUERY SELECT 'published'::text, w.id, w.public_token;
END $$;

CREATE OR REPLACE FUNCTION app.set_work_status(p_work_id uuid, p_status text)
RETURNS text LANGUAGE plpgsql SECURITY INVOKER SET search_path = public, app AS $$
...
CREATE OR REPLACE FUNCTION app.retire_work(p_work_id uuid)
RETURNS text LANGUAGE plpgsql SECURITY INVOKER SET search_path = public, app AS $$
BEGIN
...
-- borrowed by a maker.
CREATE OR REPLACE FUNCTION app.set_maker_access(p_maker_id uuid, p_action text, p_reason text)
...
BEGIN
  ...
  INSERT INTO operator_actions (actor_account_id, action, subject_type, subject_id, reason)
    VALUES (app.account_id(), p_action, 'maker', p_maker_id, p_reason);
  RETURN new_status;
END $$;
GRANT EXECUTE ON FUNCTION app.set_maker_access(uuid, text, text) TO gallery_auth;

CREATE OR REPLACE FUNCTION app.close_account()
RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, app AS $$
DECLARE acct uuid := app.account_id(); mk uuid;
BEGIN
  ...
  RETURN 'closed';
END $$;
GRANT EXECUTE ON FUNCTION app.close_account() TO gallery_auth;
